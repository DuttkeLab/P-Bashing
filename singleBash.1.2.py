...
parser.add_argument("--numSeq", "-n",help="Number of bashed sequences. will attemot to run through the bash sequence provided and find n versions of the target sequence being bashed. each bash will consist of an entirely different sequence than prior bash.")
...
numSeqs = int(args.numSeq)
zeroMotifs = args.zeroMotifs

#take the whole bash sequence
bashSeq1 = bashSeqFull

#printing status for first MT
...
while runStatus == True:
    ...
    foundSeqs += 1
    print(f'MT_sequence_{foundSeqs}:\n{newSeq}\nIterations:{iterCounter}')
    ...
    #end the running loop if the correct number of bashed sequences has been found
    #print()
    if foundSeqs < numSeqs:
        iterCounter = 0
        counter += len(targetSeq)
        continue
    else:
        runStatus = False
